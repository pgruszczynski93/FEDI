...
uchar4 __attribute__((kernel)) brick(uchar4 pixel_in, uint32_t x, uint32_t y){
    ...
    float R = (((int) (rgb.r * 255.0f * 0.003921569f * Levels)) / Levels) * 255.0f;
   	float G = (((int) (rgb.g * 255.0f * 0.003921569f * Levels)) / Levels) * 255.0f;
   	float B = (((int) (rgb.b * 255.0f * 0.003921569f * Levels)) / Levels) * 255.0f;
   	rgb.r = R/255.0f;
   	rgb.g = G/255.0f;
   	rgb.b = B/255.0f;
